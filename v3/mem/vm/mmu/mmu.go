...
func (mmu *MMU) startWalking(req *vm.TranslationReq) {
	...
	for i = 0; i < len(mmu.walkingTranslations); i++{
		...
		if reqtmp.Gmmuhit != 1 {
			...
			if IOMMU_BYPASS_FLAG == false { //IOMMU and ideal
				// hit in iommu tlb
			...
			} else if IOMMU_BYPASS_FLAG == true { //UVM
				ioactuallatency = 0
				actuallatency = mmu.walkingTranslations[i].cycleLeft - (2*pcie) - mmu.walkingTranslations[i].locallevel - mmu.walkingTranslations[i].localwait
			}
	...
	*/
	...
	if IOMMU_BYPASS_FLAG == false { //IOMMU and ideal
		...
		locallatency, mmu.latency = mmu.setuvmonlybaseline(req, cache_level, iommu_cache_level, found_in_iommutlb0, hit_in_gmmu_flag, req.DeviceID, iommuptw_wait, localptw_wait[req.DeviceID])
		iolevel_transac = 0
	}
	...
	if IOMMU_BYPASS_FLAG == false { //IOMMU and ideal
		...
		// --------------------------------------------------------
		direct_goto_os_flag = false  //disable direct goto os
	...
	fmt.Println("total requests:", ultimate_total_requests, ", hit gmmu:", req_hitin_gmmu, ", go into iommu:", req_gointo_iommu ,", goto os:",req_gointo_batch, "all compulsory", total_compulsory_miss, "all other gpu miss", total_inothergpu_miss, "ave io wait queue", total_io_wait_queue_len/startWalking_round)
	fmt.Println("iommu tlb:", req_hitin_iommutlb, ", pwc:", req_hitin_iommupwc, "pwc&pt", req_hitin_iommupwc_and_pt, ", pt:", req_hitin_iommupt, "hit total:", req_hitin_iommu, "iopwc hit rate", io_hitrate)
	fmt.Println("LATENCY breakdown: local wait:",localwaittotal, ", local memory access:", local_memory_access,", total local latency:", totallatency,", interconnect:", pcieall,
			", iommuptw total wait:", iommuptw_wait_total, "iommu memory access" , iommu_pwc_latency ,"os wait", ultimate_os_wait, ", os handle:", req_gointo_batch*UVM_BATCH_FIXED_LATENCY, ", replay", replayall ,", all latency:", ultimate_total_latency)
	
	fmt.Println("++reqs total requests", eachgpu_total_requests, "gmmu_hit", eachgpu_hit_gmmu, "goto iommu", eachgpu_goto_iommu, "goto os" , eachgpu_goto_os, "compulsory", eachgpu_compulsory_miss, "in other gpu", eachgpu_inothergpu_miss)
	fmt.Println("++iommu tlb", eachgpu_hit_io_tlb, "pwc", eachgpu_hit_io_pwc, "pt", eachgpu_hit_io_pt, "io total hit", eachgpu_hit_io_total)
	fmt.Println("++latency local wait", eachgpu_local_wait_latency, "local pwc", eachgpu_local_pwc_latency, "interconnect", eachgpu_pcieall_latency, "io wait",  eachgpu_io_wait_latency, "io pwc", eachgpu_io_pwc_latency, 
			"os wait", eachgpu_os_wait_latency, "os handle", eachgpu_oshandle_latency, "replay", eachgpu_replayall_latency, "all latency", eachgpu_alllatency)
	...
	translationInPipeline := transaction{
		...
		iommu_bypass: iommu_bypass_transc_flag,
...
func (mmu *MMU) calculateHostWaitLatency (hostThread int) int{
	...
	for i = 0; i < len(mmu.walkingTranslations); i++{
		...
		if mmu.walkingTranslations[i].os_handle_flag == true {
			...
			if reqtmp.Gmmuhit == 1  || mmu.walkingTranslations[i].iommu_tlb_hit_flag != -1 || mmu.walkingTranslations[i].iommupt_hit_flag  == 1{
				...
				if mmu.walkingTranslations[i].iommu_bypass == 1 {
					actual_host_latency = mmu.walkingTranslations[i].cycleLeft - (2*pcie) - mmu.walkingTranslations[i].locallevel*2 - mmu.walkingTranslations[i].localwait
				}else {
	...
	fmt.Println("+++==+++++os_req_num", os_req_num)
...
// 		mmusetID = int(reqtmp.VAddr / setpagesize % uint64(mmuset))
// 		if mmu.walkingTranslations[i].os_handle_flag == true {
// 			if reqtmp.Gmmuhit == 1  || mmu.walkingTranslations[i].iommu_tlb_hit_flag != -1 || mmu.walkingTranslations[i].iommu_hit_flag  == 1{
// 				actual_host_latency = 0
// 			} else {
// 				actual_host_latency = mmu.walkingTranslations[i].cycleLeft - (2*pcie) - mmu.walkingTranslations[i].locallevel*2 - mmu.walkingTranslations[i].localwait - mmu.walkingTranslations[i].iowait - mmu.walkingTranslations[i].iolevel - mmu.walkingTranslations[i].oswait
				
// 			}
// 			fmt.Println("actual_host_latency",actual_host_latency)

// 			if actual_host_latency > 0 {
// 				tmphostptw = append(tmphostptw, actual_host_latency)
// 			}
...
// 	os_wait := 0 
// 	if len(tmphostptw) < hostThread { //no need to wait
// 		os_wait = 0
// 		return os_wait
// 	}else {
// 		tmphost = append(tmphostptw[:hostThread])
// 		start_from_thread_index := hostThread
// 		times := len(tmphostptw) - hostThread

// 		for k = 0; k < times; k++ {
// 			sort.Slice(tmphost, func(i, j int) bool {return tmphost[i] < tmphost[j]})
// 			tmphost = append(tmphost, tmphostptw[start_from_thread_index] + tmphost[k])
// 			tmphostptw[start_from_thread_index] = tmphostptw[start_from_thread_index] - tmphost[k]
// 			start_from_thread_index += 1
// 		}

// 		sort.Slice(tmphost, func(i, j int) bool {return tmphost[i] < tmphost[j]})
// 		os_wait = tmphost[times]
// 	}
...
func (mmu *MMU) setuvmonlybaseline (req *vm.TranslationReq, cache_level int, iommu_cache_level int, found_in_iommutlb0 int, hit_in_gmmu_flag bool, ReqID uint64, iommuptw_wait int, localptw_wait int)(locallatency int, latency int){
