...
func (mmu *MMU) startWalking(req *vm.TranslationReq) {
	...
	for i = 0; i < len(mmu.walkingTranslations); i++{
		...
		// not hit in local, need iommu or os further handle
		if reqtmp.Gmmuhit != 1 {
	...
	*/
	...
	if IOMMU_BYPASS_FLAG == false { //IOMMU and ideal
		fmt.Println("v2.0---iommu_batch_fixed_latency:", UVM_BATCH_FIXED_LATENCY, "batchThreshold:", batchThreshold, "direct_goto_os_flag", direct_goto_os_flag)
	} else if IOMMU_BYPASS_FLAG == true { //UVM
		fmt.Println("v2.0---uvm_batch_fixed_latency:", UVM_BATCH_FIXED_LATENCY, "batchThreshold:", batchThreshold, "direct_goto_os_flag", direct_goto_os_flag)
	}

	fmt.Println("total_requests:", ultimate_total_requests, "hit_gmmu:", req_hitin_gmmu, "gotoiommu:", req_gointo_iommu ,"goto_os:",req_gointo_batch, "all_compulsory_miss", total_compulsory_miss, "all_other_gpu_miss", total_inothergpu_miss, "ave_io_wait_queue", total_io_wait_queue_len/startWalking_round)
	fmt.Println("iommu_tlb:", req_hitin_iommutlb, ", pwc:", req_hitin_iommupwc, "pwc&pt", req_hitin_iommupwc_and_pt, "pt", req_hitin_iommupt, "io_hit_total", req_hitin_iommu, "iopwc_hit_rate", io_hitrate)
	fmt.Println("LATENCY breakdown local_wait:",localwaittotal, "local_memory_access:", local_memory_access,"total_local_latency:", totallatency,"interconnect:", pcieall,
			"iommuptw_total_wait:", iommuptw_wait_total, "iommu_memory_access" , iommu_pwc_latency ,"os_wait", ultimate_os_wait, "os_handle:", req_gointo_batch*UVM_BATCH_FIXED_LATENCY, "replay", replayall ,"overall_latency:", ultimate_total_latency)
	
	fmt.Println("++each_total_requests", eachgpu_total_requests, "gmmu_hit", eachgpu_hit_gmmu, "goto_iommu", eachgpu_goto_iommu, "goto_os" , eachgpu_goto_os, "compulsory", eachgpu_compulsory_miss, "in_other_gpu", eachgpu_inothergpu_miss)
	fmt.Println("++each_iommu_tlb", eachgpu_hit_io_tlb, "pwc", eachgpu_hit_io_pwc, "pt", eachgpu_hit_io_pt, "io_total_hit", eachgpu_hit_io_total)
	fmt.Println("++each_latency_local_wait", eachgpu_local_wait_latency, "local_pwc", eachgpu_local_pwc_latency, "interconnect", eachgpu_pcieall_latency, "io_wait",  eachgpu_io_wait_latency, "iopwc", eachgpu_io_pwc_latency, 
			"os_wait", eachgpu_os_wait_latency, "os_handle", eachgpu_oshandle_latency, "replay", eachgpu_replayall_latency, "all_latency", eachgpu_alllatency)
...
func (mmu *MMU) calculateHostWaitLatency (hostThread int) int{
	...
	oswait := UVM_BATCH_FIXED_LATENCY * ((os_req_num-1)/hostThread)
	if oswait > 0 {
		return oswait
	} else {
		oswait = 0
...
func (mmu *MMU) setlatencybaseline (req *vm.TranslationReq, cache_level int, iommu_cache_level int, found_in_iommutlb0 int, hit_in_gmmu_flag bool, ReqID uint64, iommuptw_wait int, localptw_wait int)(locallatency int, latency int){
	...
	if hit_in_gmmu_flag == true || found_in_iommutlb0 != -1{
