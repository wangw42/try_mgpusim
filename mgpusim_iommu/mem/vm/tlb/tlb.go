...
func (tlb *TLB) Lastvisitlookup(vAddr uint64, PID ca.PID) (existloc int){
	existloc = -1
...
// 在哪个gpu上
func (tlb *TLB) GetLastvisit(vAddr uint64, PID ca.PID) (existloc uint64){
	existloc = 0
	for i = 0; i < len(lastvisit); i++{
		if vAddr == lastvisit[i].vaddr && PID == lastvisit[i].pid{
...
// 被某个gpu访问记录
func (tlb *TLB) Validinlocal(vAddr uint64, position uint64, PID ca.PID) bool{
...
func (tlb *TLB) IsInLocal(vAddr uint64, position uint64, PID ca.PID) bool{
	return tlb.GetLastvisit(vAddr, PID) == position
}

func (tlb *TLB) UpdateLastvisit(vAddr uint64, position uint64, PID ca.PID) {	
	loc := tlb.Lastvisitlookup(vAddr, PID)
...
func (tlb *TLB) InsertLastVisit(vAddr uint64, position uint64, PID ca.PID) {
...
func (tlb *TLB) CheckLastVisit(vAddr uint64, PID ca.PID) Last {
	loc := tlb.Lastvisitlookup(vAddr, PID)
...
func (tlb *TLB) Checkcounter(vAddr uint64, gid int, PID ca.PID) bool{
	loc := tlb.Lastvisitlookup(vAddr, PID)
...
func (tlb *TLB) lookup(now sim.VTimeInSec) bool {
	...
	*/
	...
	if strings.Contains(tlb.name, "GPU1"){
		gid = 1	
	}else if strings.Contains(tlb.name, "GPU2"){
		gid = 2
	}else if strings.Contains(tlb.name, "GPU3"){
		gid = 3
	}else if strings.Contains(tlb.name, "GPU4"){
		gid = 4
	}else if strings.Contains(tlb.name, "GPU5"){
		gid = 5
	}else if strings.Contains(tlb.name, "GPU6"){
		gid = 6
	}else if strings.Contains(tlb.name, "GPU7"){
		gid = 7
	}else if strings.Contains(tlb.name, "GPU8"){
		gid = 8
	}
...
func (tlb *TLB) Tablelookup(vAddr uint64,PID ca.PID) (existloc int){
	existloc = -1
	for i = 0; i < len(table); i++{
...
func (tlb *TLB) TableInsert(PID ca.PID, gid int, vAddr uint64) bool{
	for i = 1; i < 9; i++{
		if i == gid{
			store[i] = 1
			access[i] = 1
...
func (tlb *TLB) TableUpdate(gid int, vAddr uint64,PID ca.PID) bool {
	for i = 0; i < len(table); i++{
...
func (tlb *TLB) TableDelete(vAddr uint64,PID ca.PID) bool{
	for i=0; i<len(table); i++{	
		if vAddr == table[i].vaddr && PID == table[i].pid{
...
*/
func (tlb *TLB) FlushMMUTLB(VAddr uint64, PID ca.PID) bool{
...
func (tlb *TLB) InsertMMUTLB(VAddr uint64, PID ca.PID) bool{
	mmusetID = int(VAddr / setpagesize % uint64(mmuset))	
	for i = 0; i < mmuway; i++{
...
func (tlb *TLB) LookupMMUTLB(VAddr uint64, mmusetID int, PID ca.PID)(existloc int) {
	existloc = -1
	for i = 0; i < mmuway; i++ {
		if mmutlb[mmusetID][i].addr == VAddr && mmutlb[mmusetID][i].pid == PID{
			existloc  = i
...
func (tlb *TLB) InsertL2TLB(gid int, VAddr uint64, PID ca.PID, l2setID int) bool {
...
func (tlb *TLB) LookupL2TLB(gid int, VAddr uint64, l2setID int, PID ca.PID)(existloc int) {
	existloc = -1
	for i = 0; i < l2way; i++ {
...
func (tlb *TLB) DeleteL2TLB(gid int, VAddr uint64, PID ca.PID) bool {
